...
# Function to perform the simulation for different sample sizes
def perform_simulation(sample_sizes):
    results_list = []
    
    for n_samples in sample_sizes:
        x_data, y_data = generate_data(n_samples, theta_true, beta_true, c_true, omega_true)
        
        # Perform MLE on the data
        ...
        results_list.append({
            ...
            'C MLE': c_mle,
            'Omega MLE': omega_mle,
            'Theta HDI Lower': theta_hdi[0],
            'Theta HDI Upper': theta_hdi[1],
            'Beta HDI Lower': beta_hdi[0],
            'Beta HDI Upper': beta_hdi[1],
            'C HDI Lower': c_hdi[0],
            'C HDI Upper': c_hdi[1],
            'Omega HDI Lower': omega_hdi[0],
            'Omega HDI Upper': omega_hdi[1]
        })
...
results_list = perform_simulation(sample_sizes)

# Convert results to DataFrame and save to CSV
results_df = pd.DataFrame(results_list)
results_df.to_csv('mle_hdi_simulation_resultsALLSAMPLES.csv', index=False)
